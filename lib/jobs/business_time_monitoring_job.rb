class BusinessTimeMonitoringJob
  def call
    ...
    dogrun_places.each do |dogrun_place|
      ...
      p "*" * 10
      p "opening_time:"
      p Time.zone.parse("#{Time.zone.now.strftime('%Y-%m-%d')} #{dogrun_place.opening_time}").strftime('%Y%m%d%H%M').to_i
      p "*" * 10
      p "closing_time:"
      p Time.zone.parse("#{Time.zone.now.strftime('%Y-%m-%d')} #{dogrun_place.closing_time}").strftime('%Y%m%d%H%M').to_i
      p "*" * 10
