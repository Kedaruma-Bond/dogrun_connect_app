class TogoInuShitsukeHirobaEncountGeneratorJob
  def call
    ActiveRecord::Base.transaction do
      if Rails.cache.exist?('togo_inu_shitsuke_hiroba_previous_dogs_id')
        previous_dogs_id = Rails.cache.read('togo_inu_shitsuke_hiroba_previous_dogs_id')
      ...
      encount_dogs_user_id_ary.each do |user_id|
        ...
        present_entry = Entry.joins(:dog).where(dog: { user_id: user_id }).where(created_at: Time.zone.now.all_day).last
