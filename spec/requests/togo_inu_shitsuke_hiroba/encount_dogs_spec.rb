...
RSpec.describe TogoInuShitsukeHiroba::EncountDogsController, type: :request do
    ...
    describe 'ログインしているとき' do
      ...
      context 'ログインしているアカウントに紐づいたdataをeditする場合' do
        example '正常なレスポンスが返りackされること' do
          get edit_togo_inu_shitsuke_hiroba_encount_dog_path(encount_dog_1)
          expect(response).to have_http_status(:success)
      ...
      context '別のアカウントに紐づいたdataをeditする場合' do
        example '表示されないこと' do
          get edit_togo_inu_shitsuke_hiroba_encount_dog_path(encount_dog_2)
  ...
  describe 'PATCH #update' do
    describe 'ログインしているとき' do
      ...
      context 'ログインしているアカウントに紐づいたdataをupdateする場合' do
        example 'データの更新され一覧ページにリダイレクトされること' do
          patch togo_inu_shitsuke_hiroba_encount_dog_path(encount_dog_1), params: { encount_dog: { memo: 'Updated memo' } }
      ...
      context '別のアカウントに紐づいたdataをupdateする場合' do
        example '更新できないこと' do
          patch togo_inu_shitsuke_hiroba_encount_dog_path(encount_dog_2), params: { encount_dog: { memo: 'Updated memo' } }
    ...
    describe 'ログインしていないとき' do
      example 'root画面にリダイレクトされエラーメッセージが表示されること' do
        patch togo_inu_shitsuke_hiroba_encount_dog_path(encount_dog_1)
        expect(response).to redirect_to(root_path)
        expect(flash[:error]).to eq(I18n.t('defaults.require_login'))
      end
    end
    ...
    describe 'ログインしているとき' do
      ...
      context 'ログインしているアカウントに紐づいたdataを削除する場合' do
        example 'レコードが削除され一覧ページにリダイレクトされること' do
          expect {
            delete togo_inu_shitsuke_hiroba_encount_dog_path(encount_dog_1)
      ...
      context '別のアカウントに紐づいたdataを削除する場合' do
        example '削除できないこと' do
          expect {
            delete togo_inu_shitsuke_hiroba_encount_dog_path(encount_dog_2)
