...
RSpec.describe Reon::EmbedsController, type: :request do
    ...
    describe 'ログインしているとき' do
      ...
      context 'postのtypeがembedでない場合' do
        example 'dogrunのtop pageにリダイレクトされること' do
          get new_reon_embed_path(id: post_article.id)
      ...
      context 'postのuserがcurrent_userでない場合' do
        example 'dogrunのtop pageにリダイレクトされること' do
          get new_reon_embed_path(id: post_embed_other_user.id)
          expect(response).to redirect_to(reon_top_path)
          expect(flash[:error]).to eq(I18n.t('defaults.illegal_route'))
        end
      end
      ...
      context 'postのdogrun_placeが正しくない場合' do
        example 'dogrunのtop pageにリダイレクトされること' do
          get new_reon_embed_path(id: post_embed_other_dogrun.id)
          expect(response).to redirect_to(reon_top_path)
          expect(flash[:error]).to eq(I18n.t('defaults.illegal_route'))
        end
      end
      ...
      context 'postのtypeがembedでuserとdogrun_placeが正しいとき' do
        example '正常なレスポンスが変えること' do
          get new_reon_embed_path(id: post_embed.id)
    ...
    describe 'ログインしているとき' do
      ...
      context '入力されたパラメータが正常な場合' do
        example '新規作成されること' do
          expect {
            post reon_embed_path(post_embed),
            params: {
              embed: {
                embed_type: "twitter",
