  ...
  describe 'GET #edit' do
    describe 'ログインしているとき' do
      ...
      example '正常なレスポンスが返りackされること' do
        get edit_reon_encount_dog_path(encount_dog)
        expect(response).to have_http_status(:success)
  ...
  describe 'PATCH #update' do
    describe 'ログインしているとき' do
      ...
      example 'データの更新され一覧ページにリダイレクトされること' do
        patch reon_encount_dog_path(encount_dog), params: { encount_dog: { memo: 'Updated memo' } }
    ...
    describe 'ログインしていないとき' do
      example 'root画面にリダイレクトされエラーメッセージが表示されること' do
        patch reon_encount_dog_path(encount_dog)
        expect(response).to redirect_to(root_path)
        expect(flash[:error]).to eq(I18n.t('defaults.require_login'))
      end
    end
  ...
  describe 'DELETE #destroy' do
    ...
    describe 'ログインしているとき' do
      ...
      example 'レコードが削除され一覧ページにリダイレクトされること' do
        expect {
          delete reon_encount_dog_path(encount_dog)
