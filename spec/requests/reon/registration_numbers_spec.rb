...
RSpec.describe Reon::RegistrationNumbersController, type: :request do
  ...
  describe 'GET #entries_record_analysis' do
    describe 'ログインしているとき' do
      before do
        reon_log_in_as(general)
        get entries_record_analysis_reon_registration_number_path(registration_number)
      end

      example '正常にレンダリングされること' do
        expect(response).to have_http_status(:ok)
        expect(response).to have_http_status(:ok)
        expect(assigns(:notation_of_registration_number)).to eq(I18n.t('reon.registration_number'))
      end 
    end
